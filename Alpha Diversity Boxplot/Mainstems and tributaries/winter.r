...
library(ggsci)
library(ggprism)
feast = read.csv(file = "winter.csv",sep=',',header=T)
colnames(feast)
...
y.values <- sapply(split(feast.p, feast.p$Habitat), function(x){max(sapply(split(x, x$name), function(xx){boxplot(x$value, plot=F)$stats[5, ]}))})
...
p
ggsave("winter.pdf",plot = p,width = 600,height = 300,units = "mm")
