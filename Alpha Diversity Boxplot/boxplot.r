...
library(ggsci)
library(ggprism)
feast = read.csv(file = "alpha.csv",sep=',',header=T)
colnames(feast)
...
y.values <- sapply(split(feast.p, feast.p$Habitat), function(x){max(sapply(split(x, x$name), function(xx){boxplot(x$value, plot=F)$stats[5, ]}))})+700
...
p
ggsave("source.pdf",plot = p,width = 600,height = 300,units = "mm")
