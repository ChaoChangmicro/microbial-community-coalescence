...
otu <- read.csv('summer.csv', row.names = 1, sep = ',', stringsAsFactors = FALSE, check.names = FALSE)
...
p <- ggplot(data = nmds_dis_site, aes(x = MDS1, y = MDS2)) +
  ...
  geom_point(aes(fill=cluster, size = 1, shape = site)) +
  ...
  guides(shape = guide_legend(order = 2, override.aes=list(size = 3)),
         ...
         size = FALSE)+ annotate(geom="text", x=-0.4, y=0.45, label="Stress = 0.13")
p
ggsave("summer_nmds.pdf",plot = p,width = 200,height = 200,units = "mm")
