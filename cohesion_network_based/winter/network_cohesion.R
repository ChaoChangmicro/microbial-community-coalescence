#read table
spe <- read.csv('winter.csv', row.names = 1, sep = ',', check.names = FALSE)
genus_t<-t(spe)
rel <- genus_t/rowSums(genus_t)
spe<- as.data.frame(t(rel))

#Network adjacency matrix
g <- read.csv('winter_network.csv', row.names = 1, sep = ',', check.names = FALSE)
g[g== 0] <- NA
spe <- spe[row.names(g),]
...
#write
write.table(r, 'winter_connectedness.txt', sep = '\t', row.names = FALSE, quote = FALSE)
write.table(C, 'winter_cohesion.txt', sep = '\t', row.names = FALSE, quote = FALSE)
