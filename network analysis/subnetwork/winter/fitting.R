...
plot_correlation_linearfitting <- ggplot(data = data, aes(x = Coalescence, y = Value, group = type, color = type)) + 
 ...
 # stat_poly_eq(use_label(c("eq", "adj.R2", "P")),formula = formula, size = 4) +
  ...
  theme(panel.grid = element_blank(),
        legend.background = element_rect(fill = "white", color = "white"),  # 设置图例背景色为白色
        legend.position = "none",  # 设置图例位置为"none"
        axis.text = element_text(size = 13, color = "#000000"),
        axis.title = element_text(size = 15),  # 调整坐标轴标题字体大小
        plot.title = element_text(hjust = 0.5, size = 20),
        legend.title = element_blank(),  # 隐藏图例标题
        legend.text = element_text(size = 10),
        plot.subtitle = element_text( size = 15),  # 调整副标题字体大小
        strip.text = element_text(size = 15)  # 调整分面标题字体大小
  ) + 
  facet_wrap(~type, scales = "free_y")+
  labs(x = "Multi-source community coalescence", y = "Network topological properties", subtitle = "Winter-Water")
plot_correlation_linearfitting
p=plot_correlation_linearfitting

ggsave("Winter_water.pdf",plot = p,width = 400,height = 300,units = "mm")
ggsave("Winter_biofilm.pdf",plot = p,width = 400,height = 300,units = "mm")
ggsave("Winter_soil.pdf",plot = p,width = 400,height = 300,units = "mm")
ggsave("Winter_sediment.pdf",plot = p,width = 400,height = 300,units = "mm")
